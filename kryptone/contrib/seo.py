...
class TFIDFProcessor:
    ...
    def filter_tokens_by_tfidf(self, document_idx, top_n=None, threshold=None):
        ...
        if threshold is not None:
            filtered_terms = {
                term: score for term, score in term_scores.items() if score >= threshold
            }
        elif top_n is not None:
            filtered_terms = dict(
                sorted(term_scores.items(), key=lambda x: x[1], reverse=True)[:top_n]
            )
        else:
    ...
    def preprocess_text_with_tfidf(self, keep_top_n=None, min_tfidf=None):
        ...
        for i in range(len(self.documents)):
            filtered_tokens = self.filter_tokens_by_tfidf(
                i, top_n=keep_top_n, threshold=min_tfidf
            )
...
class SEOMixin(TextMixin):
    ...
    def page_speed_script(self):
        path = settings.GLOBAL_KRYPTONE_PATH.joinpath("data", "js", "page_speed.js")
    ...
    def calculate_word_frequency(self, tokens):
        ...
        sorted_frequency = sorted(frequency_values, key=lambda x: x[1], reverse=True)[
            0:10
        ]
    ...
    def audit_structure(self, audit):
        ...
        has_head_title = all(
            [self.get_page_title is not None, self.get_page_title != ""]
        )
...
class EmailMixin(TextMixin):
    ...
    def find_emails_from_text(self):
        ...
        emails_from_text = map(self.identify_email, text.split(" "))
