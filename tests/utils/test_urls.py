...
class TestUrl(unittest.TestCase):
    ...
    def test_structure(self):
        ...
        self.assertTrue(self.instance.is_same_domain("https://www.defacto.com/fr-ma"))
    ...
    def test_contains(self):
        urls = [URL("http://example.com"), URL("http://example.com/1")]
...
class TestURLIgnoreTest(unittest.TestCase):
    ...
    def test_ignore_paths(self):
        ...
        url_count = len(list(filter(lambda u: "/femmes/vetements" in u, self.urls)))

        self.assertGreater(
            url_count, 0, "Test URLs must contain at least one matching URL"
        )
        ...
        self.assertEqual(
            total_tests, url_count, "All matching URLs should have been tested"
        )

    def test_ignore_paths_regex(self):
        instance = URLIgnoreRegexTest("test-name", regex=r"\/minijupe\-denim")

        url_count = len(list(filter(lambda u: "/minijupe-denim" in u, self.urls)))
...
class TestMultipleURLManager(unittest.TestCase):
    ...
    def test_add_urls(self):
        ...
        self.manager.add_urls([url1, url2])

        self.assertIn(url1, self.manager._urls_to_visit)
        self.assertIn(url2, self.manager._urls_to_visit)

    def test_ignore_image_urls(self):
        image_url = URL("http://example.com/image.jpg")
        self.manager.add_urls([image_url])

        self.assertNotIn(image_url, self.manager._urls_to_visit)

    def test_sort_urls(self):
        url1 = URL("http://example.com/b")
        url2 = URL("http://example.com/a")

        self.manager.add_urls([url1, url2])

        sorted_urls = list(self.manager._urls_to_visit)
        self.assertEqual(sorted_urls[0], url2)
        self.assertEqual(sorted_urls[1], url1)

    def test_run_custom_filters(self):
        # Define a custom filter that ignores URLs containing 'ignore'
        def custom_filter(url):
            return "ignore" in str(url)

        self.manager.custom_url_filters.append(custom_filter)

        url1 = URL("http://example.com/keep")
        url2 = URL("http://example.com/ignore")

        urls_kept = self.manager.run_url_filters([url1, url2])

        self.assertIn(url1, urls_kept)
        self.assertNotIn(url2, urls_kept)
    ...
    def test_populate(self):
        ...
        self.manager.start_url = None
        self.manager.populate(start_urls)

        for url in start_urls:
            self.assertIn(URL(url), self.manager._urls_to_visit)
